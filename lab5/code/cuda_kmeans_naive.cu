                    ...
                    int    clusterId)
{
	...
	ans += (objects[numCoords * objectId + i] - clusters[numCoords * clusterId + i]) *
	       (objects[numCoords * objectId + i] - clusters[numCoords * clusterId + i]);
    }
						...
						int blockSize)
{
	...
	fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n", "Naive", blockSize, timing/loop, timer_min, timer_max, gpu_timer/loop, cpu_timer/loop, gpu_cpu_timer/loop, cpu_gpu_timer/loop);
	fclose(fp); 
