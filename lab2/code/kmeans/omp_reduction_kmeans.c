            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    #pragma omp parallel for shared(local_newClusters, local_newClusterSize) private(k)
	...
	#pragma omp parallel for shared(newClusters, newClusterSize) private(i, j)
         ...
         */
	#pragma omp parallel for shared(local_newClusters, local_newClusterSize) private(i, j, k)
        for (k=0; k<nthreads; k++)
        {
            for (i=0; i<numClusters; i++) {
                for (j=0; j<numCoords; j++)
