            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
	...
	#pragma omp parallel for shared(newClusters, newClusterSize) private(i, j)
        ...
        /* 
         * DONE: Detect parallelizable region and use appropriate OpenMP pragmas
         */
	#pragma omp parallel for shared(newClusters, newClusterSize) private(i, j, index) reduction(+:delta)	
        for (i=0; i<numObjs; i++) {
            ...
            /*
             * DONE: enforce atomic access to shared "newClusterSize" array
             */
	    #pragma omp atomic
            	newClusterSize[index]++;
            for (j=0; j<numCoords; j++)
                /*
                 * DONE: enforce atomic access to shared "newClusters" array
                 */
		#pragma omp atomic
