...
int main(int argc, char ** argv) {
    ...
    for (t=0;t<T;t++) {
		...
		if (north != MPI_PROC_NULL) {
			MPI_Isend(&(u_previous[1][1]), 1, row, north, 0, MPI_COMM_WORLD, &requests[requests_count++]);
			MPI_Irecv(&(u_previous[0][1]), 1, row, north, 1, MPI_COMM_WORLD, &requests[requests_count++]);
    	}

		if (south != MPI_PROC_NULL) {
			MPI_Isend(&(u_previous[local[0]][1]), 1, row, south, 1, MPI_COMM_WORLD, &requests[requests_count++]);
            MPI_Irecv(&(u_previous[local[0]+1][1]), 1, row, south, 0, MPI_COMM_WORLD, &requests[requests_count++]);
    	}

		if (west != MPI_PROC_NULL) {
			MPI_Isend(&(u_previous[1][1]), 1, column, west, 2, MPI_COMM_WORLD, &requests[requests_count++]);
            MPI_Irecv(&(u_previous[1][0]), 1, column, west, 3, MPI_COMM_WORLD, &requests[requests_count++]);
    	}

		if (east != MPI_PROC_NULL) {
			MPI_Isend(&(u_previous[1][local[1]]), 1, column, east, 3, MPI_COMM_WORLD, &requests[requests_count++]);
            MPI_Irecv(&(u_previous[1][local[1]+1]), 1, column, east, 2, MPI_COMM_WORLD, &requests[requests_count++]);
    	}
    ...
    if (rank==0) {
        printf("Jacobi X %d Y %d Px %d Py %d Iter %d ComputationTime %lf MessagingTime %lf TotalTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,msg_time,total_time,U[global[0]/2][global[1]/2]);
